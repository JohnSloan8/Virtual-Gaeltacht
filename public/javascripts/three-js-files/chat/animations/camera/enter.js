...
export default function cameraEnter(amount=0.7, duration=3000, easing="cubicIn") {
	...
	let cameraEnterPositionTween = new TWEEN.Tween(camera.position).to({z: posRot[participantNamesArray.length][0].z, y: posRot[participantNamesArray.length].camera.y+0.05 }, duration)
	.easing(easingDict["quinticIn"])

	let cameraMeGroupEnterTween = new TWEEN.Tween(cameraMeGroup.position).to({z: posRot[participantNamesArray.length][0].z - 0.5 }, duration*2)
	.easing(easingDict["cubicInOut"])
	.start()
